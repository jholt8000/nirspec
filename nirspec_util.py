...
class NirspecBookkeeping(object):
    ...
    def __init__(self, fits_name, outpath="", verbose=True):
        """ Nirspec_bookkeeping needs only the FITS file name
        :param fits_name: string
            Name of input science FITS file
        :param outpath: string (optional)
            Location for output files
        :param verbose: bool
            Extra information about reduction
        """
    ...
    def close_logger(self):
        ...
        self.logger must be setup for this to work
        """
...
def make_nirspec_final_fits_and_plots(allreduceobj, order_num, sciorder, lineobj, traceobj, flatobj, sciobj):
    """
    Creates final reduction outputs using all objects determined during reduction for each order
   ...
   Parameters:
    ...
    if allreduceobj.write_plots:
        pl.savefig(allreduceobj.outpath + allreduceobj.sciname + 'sky_order_' + str(order_num) + '.png',
                   bbox_inches=0)
    # if write_fits:
    ...
    if allreduceobj.write_plots:
        pl.savefig(allreduceobj.outpath + allreduceobj.sciname + 'sky_order_' + str(order_num) + '.png',
                   bbox_inches=0)
    # if write_fits:
    # pf.writeto(outpath+sciname+'extracted_order_num'+str(order_num)+'.fits',np.array((sciorder.dx_2dfit,ex2)),
    # reductionobj.sciheader,output_verify='warn',clobber=True)
    ...
    if allreduceobj.write_plots:
        pl.savefig(allreduceobj.outpath + allreduceobj.sciname + 'crosscut_order' + str(order_num) + '.png',
                   bbox_inches=0)
    if allreduceobj.write_fits:
        # pf.writeto(allreduceobj.outpath+allreduceobj.sciname+'crosscut_order'+str(order_num)+'.fits',
        # np.array([sciorder.crosscut]),allreduceobj.sciheader,output_verify='warn',clobber=True)
    ...
    pl.imshow(sciorder.rectified, origin='lower',
              extent=[sciorder.dx_2dfit[0], sciorder.dx_2dfit[-1], 0, sciorder.rectified.shape[0]], aspect='auto')
    pl.xlabel("$\mu$")
    if allreduceobj.write_plots:
        pl.savefig(allreduceobj.outpath + allreduceobj.sciname + 'rectified_order' + str(order_num) + '.png',
                   bbox_inches=0)
    if allreduceobj.write_fits:
        pf.writeto(allreduceobj.outpath + allreduceobj.sciname + 'rectified_order' + str(order_num) + '.fits',
    ...
    if True:
        ...
        if allreduceobj.write_plots:
            pl.savefig(allreduceobj.outpath + allreduceobj.sciname + 'allflat' + str(order_num) + '.png',
                       bbox_inches=0)
        ...
        if allreduceobj.write_plots:
            pl.savefig(allreduceobj.outpath + allreduceobj.sciname + 'allsci' + str(order_num) + '.png',
                       bbox_inches=0)
...
class NirspecHeader(object):
    ...
    :param header: astropy header object
        header should be opened using the typical astropy methods
    """
    ...
    def get_theory_order_pos(self, order_num, a_to_mu=True):
        """
        ...
        :param order_num: int
            order number
        :param a_to_mu:
            Convert Anstroms to microns
        """
        filter_name = self.header['filtname']

        if 'NIRSPEC-7' in filter_name:
            c1 = 0.24792775
            c2 = -35906.947
            y0 = 15955.4515
            r1 = 0.23482994
            ...
            print 'I cannot reduce filter = ' + filter_name
            return 0, 0, 0
        ...
        wl50 = (852800. * np.sin(np.radians(self.header['echlpos'])) -
                24.13 * (512 - x1) * np.cos(np.radians(self.header['echlpos']))) / order_num

        wl50_2 = (852800. * np.sin(np.radians(self.header['echlpos'])) -
                  24.13 * (512 - 50) * np.cos(np.radians(self.header['echlpos']))) / order_num
    ...
    def sanity_check(self, flatheader):
        ...
        filter names and slit name
        :param flatheader: object
            header of the matching flat FITS file, opened using astropy.io.fits
...
def get_actual_order_num_pos(edges, theory, threshold):
    """
    :param edges:
    :param theory:
    :param threshold:
