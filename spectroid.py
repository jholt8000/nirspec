...
def spectroid(data_array, spw=10, bkw=30, dloc=926, trace_mean=True, 
    ...
    ''' Find centroids for each column in data_array -- only works horizontally 
     ...
     trace_last=False: if trace_last is true and centroid found is 
          >trace_delta*abs(previous centroid found) then use previous centroid  
           ...
           discontinuities
           trace_last will take precidence over trace_mean if both are set to True
    ...
    '''
    #initalize "center of mass" array to be populated with centroids
    ...
    for i in range(xmin,xmax):
        ...
        if (bkw > 0): # Subtract background
            ...
            if bkmax > data_array.shape[0]: bkmax = data_array.shape[0]-1
            if bkmin < 0: bkmin = 0
        ...
        else: bk_mean=0.0 # do not subract background value
        ...
        if np.abs(cm[i]-cm[i-1]) > trace_delta: 
          ...
          if trace_mean:
            ...
            if len(cm) > 4: 
                ...
                kmax = len(cm) 
            else: kmax = 2 #use the first one found
