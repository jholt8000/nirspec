...
class Trace_order_utils(object):
    ...
    def __init__(self, reduction, flatobj, sciobj):
    ...
    def trace_order(self):
        ...
        self.bottom_spectroid = np.array(0)
        ...
        #start with padding amount for this filter and slit
        self.padding = self.reduction.data_dict['padding']
        ...
        self.lhs_top_theory, self.lhs_bot_theory, self.dx = self.reduction.nh.get_theory_order_pos(self.reduction.order_num)
   
        self.reduction.logger.info('Theory -- left bot = '+str(int(self.lhs_bot_theory))+\
        ...
        if self.lhs_top_theory < self.sciobj.data.shape[0]+fudge_constants.Nirspec_fudge_constants.total_chip_padding and self.lhs_bot_theory > -fudge_constants.Nirspec_fudge_constants.total_chip_padding: 
            ...
            self.lhs_top = self.determine_lhs_edge_pos(self.flatobj.tops, self.lhs_top_theory, self.reduction.data_dict)                               
               
            self.lhs_bot = self.determine_lhs_edge_pos(self.flatobj.bots, self.lhs_bot_theory, self.reduction.data_dict)

            self.reduction.logger.info('Measured -- left bot = '+str(int(self.lhs_bot)))
            self.reduction.logger.info('            left top = '+str(int(self.lhs_top)))
            ...
            if self.lhs_top > self.lhs_bot:
                ...
                # top_spectroid is the centroid fit to the top of the order
                # bft is bad fit top = number of times spectroid had to self-correct
                self.top_spectroid, bft = spectroid(self.flatobj.tops, dloc = self.lhs_top, spw = self.reduction.data_dict['spw'], 
                                    bkw = 0, trace_mean = True, trace_last = False, 
                                    trace_delta = self.reduction.data_dict['trace_delta']) 
                                    
                self.reduction.logger.info('had to self correct on top = '+str(bft)+' times ')
                ...
                self.bottom_spectroid, bfb = spectroid(self.flatobj.bots, dloc = self.lhs_bot, spw = self.reduction.data_dict['spw'], 
                                            bkw = 0, trace_mean = True, trace_last = False, 
                                            trace_delta = self.reduction.data_dict['trace_delta'])
                                            
                self.reduction.logger.info('had to self correct on bottom = '+str(bfb)+' times ')
                if bfb > fudge_constants.Nirspec_fudge_constants.badfit_limit: self.traced_bot = False
                else: self.traced_bot = True
                
                try: self.lhs_bot = self.bottom_spectroid[1]
                ...
                if self.traced_top and self.traced_bot:
                        ...
                        self.avg_spectroid = (self.top_spectroid + self.bottom_spectroid) / 2.        

                elif self.traced_top:  
                    self.reduction.logger.info('using only top curve ')
                    self.avg_spectroid = self.top_spectroid-((self.lhs_top-self.lhs_bot)/2)+1.
                elif self.traced_bot:
                    self.reduction.logger.info('using only bottom curve ')
                    self.avg_spectroid = self.bottom_spectroid+((self.lhs_top-self.lhs_bot)/2)+1.
                    
                else:
                    self.reduction.logger.info('could not trace order '+str(self.reduction.order_num))
                    self.trace_success = False
                    self.lhs_top = self.lhs_top_theory
                    
     
            else:
                self.reduction.logger.info('top of order cannot be below bottom')
                self.reduction.logger.info ('skipping order: '+str(self.reduction.order_num))
                self.trace_success = False
                self.lhs_top = self.lhs_top_theory

        else:
            self.reduction.logger.info('order: '+str(self.reduction.order_num)+' not on detector  ')
            self.trace_success = False
            self.lhs_top = self.lhs_top_theory
    ...
    def fudge_padding(self):
           # we shouldn't have to do this if the initial number is correct
            #if order is very curved, add a bit more reduction.padding to 
    ...
    def smooth_spectroid(self):
            self.reduction.logger.info('fitting a polynomial function from spectroid ')
    ...
    def shift_order(self): 
            print 'bs=',self.bottom_spectroid[0]
            print 'self.reduction.paadding=',self.padding
            if self.bottom_spectroid[0] > self.padding: 
                ...
                # centroid top -> ct, centroid middle -> cm, centroid bottom->cb
                 self.top_spectroid = self.top_spectroid - self.bottom_spectroid[0] + self.padding
                 self.avg_spectroid = self.avg_spectroid - self.bottom_spectroid[0] + self.padding
                 self.bottom_spectroid = self.bottom_spectroid - self.bottom_spectroid[0] + self.padding
                 
                 self.order_shifted=True
             
            else: self.order_shifted=False
    ...
    def shift_order_back(self):
        if self.order_shifted:
            ...
            self.bottom_spectroid=self.bottom_spectroid - self.padding + self.lhs_bot
    ...
    def determine_lhs_edge_pos(self, edges, theory_lhs, data_dict):
        ...
        lhs_edge_pos = self.reduction.nh.get_actual_order_pos(edges, theory_lhs, 
                                          data_dict['threshold'])
 
        self.reduction.logger.info('found a peak at '+str(lhs_edge_pos))
        ...
        if not found_edge:
            ...
            self.reduction.logger.info('searching for top edge on a fainter flat')
            self.reduction.logger.info('  ---> this might affect the rectification')
            ...
            if not found_top:
                self.reduction.logger.info('Flat is too faint in this order ')
                self.reduction.logger.info('Cannot find order location')
