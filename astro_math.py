...
def cheb_fit(matchesohx,matchesdx,dx): 
        '''
          not used
        '''    
        if len(matchesohx) > 4: chebord=3
        else: chebord=2
        
        # c is Legendre series coefficients, chebord is degree of the fitting serious
        c,[residuals,rank,singular_values,rcond]=np.polynomial.chebyshev.chebfit(matchesdx,matchesohx,chebord,full=True)
...
def fit_poly(cm, xes='default', deg=4):
    ...
    if xes == 'default': xes=np.arange(len(cm))
    if deg==4:
...
def newpoint2(popt, new_order):
    ...
    new_disp=popt[0]*new_order**2 + popt[1]*new_order + popt[2]*new_zeropoint**2 + popt[3]*new_zeropoint + popt[4]
...
def wavelength_solution_2d(all_matches):
    ...
    if len(all_matches) > 0:
        ...
        coeffs=fitPlaneLTSQ(dxs,order_array,ohs)
        ...
        nx,ny=100,100
        xx,yy = np.meshgrid(np.linspace(dxs.min()-5, dxs.max()+5, nx), np.linspace(order_array.min()-0.005, order_array.max()+0.005, ny))
        ...
        pl.clf()
        i=0
        colors = ['r','g','c','k','m','b','y','r','g','c','k','m','b','y','r','g','c','k','m','b','y']
        #all_matches.append((lineobj.matchesdx, lineobj.matchesohx, lineobj.matchesidx, order))
...
def apply_2d_wavelength_solution(p0,order,all_disp_sol):
               ...
               #self.logger.info('Wavelength dispersion '+str(new_disp)+' and offset '+str(new_offset)+' applied to order '+str(order))
