...
class BaseArray(object):     
    ...
    def find_peak(self, order_shifted=False):
        ...
        if order_shifted: crosscut = self.data.sum(axis = 1)
        else: crosscut = self.data[:30]
            
        if crosscut.any(): peak = np.argmax(crosscut)
        else: peak = 0.0
        self.peak = peak
...
class SciArray(BaseArray):
    ...
    def get_sky(self, peak, sky_range_bot, sky_range_top):
        ...
        try: skys1 = np.sum(self.data[peak+i, :] for i in sky_range_bot)
        except: 
            skys1 = [0]
            sky_height_bot = 0.
        
        try: skys2 = np.sum(self.data[peak+i, :] for i in sky_range_top)
        except: 
            skys2 = [0]
            sky_height_top = 0.  
        ...
        if sky_height_top > 0. and sky_height_bot > 0.:      
            ...
            self.skys = Nirspec_fudge_constants.badval
    ...
    def sum_extract(self, ext_range, sky_range_bot, sky_range_top):
        ...
        try:
            ...
            extracted = extracted/len(ext_range)
            #sets self.skys            
...
class FlatArray(BaseArray):
    ...
    def normalize(self, on_order=1.0, off_order=1.0, mask = True, instr = "NIRSPEC"):
        ...
        off_order: array
           pixel array of same size of self.data with off-order pixels masked
        mask: bool
           when mask=True, off-order pixels are not used in compuatation of mean
        instr: str
    ...
    def trace_order(tops, bots, nh, data_dict, order, logger, sciobj):
        ...
        else: traced_bot = True
        
        try: lhs_bot = cb[1]
        except: traced_bot = False               
